--Class Definitions
	...
	local itemClass = item_tome_of_strength

	--Passive Bonus Modifier
	modifier_tome_str = class({})
	local modifierClass = modifier_tome_str
	local modifierName = 'modifier_tome_str'
	LinkLuaModifier(modifierName, "items/tome_of_strength", LUA_MODIFIER_MOTION_NONE)

	--Passive Bonus Modifier
	modifier_update_str = class({})
	local updateModClass = modifier_update_str
	local updateModName = 'modifier_update_str'
	LinkLuaModifier(updateModName, "items/tome_of_strength", LUA_MODIFIER_MOTION_NONE)

	function updateModClass:GetTexture()
		return "item_book_of_strength"
	end
...
function itemClass:OnSpellStart()
	...
	local str_per_use = self:GetSpecialValueFor("str_per_use")

	caster:AddNewModifier(caster, self, modifierName, { duration = -1})

	--Tooltip Cosmetic
	if not caster:HasModifier(updateModName) then 
		caster:AddNewModifier(caster, self, updateModName, { duration = -1, str = str_per_use})
...
function modifierClass:OnCreated()
	...
	self.bonus_str = self:GetAbility():GetSpecialValueFor( "str_per_use" )
