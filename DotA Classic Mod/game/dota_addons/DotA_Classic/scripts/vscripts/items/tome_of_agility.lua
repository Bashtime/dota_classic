--Class Definitions
	...
	local itemClass = item_tome_of_agility

	--Passive Bonus Modifier
	modifier_tome_agi = class({})
	local modifierClass = modifier_tome_agi
	local modifierName = 'modifier_tome_agi'
	LinkLuaModifier(modifierName, "items/tome_of_agility", LUA_MODIFIER_MOTION_NONE)

	--Passive Bonus Modifier
	modifier_update_agi = class({})
	local updateModClass = modifier_update_agi
	local updateModName = 'modifier_update_agi'
	LinkLuaModifier(updateModName, "items/tome_of_agility", LUA_MODIFIER_MOTION_NONE)

	function updateModClass:GetTexture()
		return "item_book_of_agility"
	end
...
function itemClass:OnSpellStart()
	...
	local agi_per_use = self:GetSpecialValueFor("agi_per_use")

	caster:AddNewModifier(caster, self, modifierName, { duration = -1})

	--Tooltip Cosmetic
	if not caster:HasModifier(updateModName) then 
		caster:AddNewModifier(caster, self, updateModName, { duration = -1, agi = agi_per_use})
...
function modifierClass:OnCreated()
	...
	self.bonus_agi = self:GetAbility():GetSpecialValueFor( "agi_per_use" )
