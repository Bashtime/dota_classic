--Class Definitions
	...
	item_euls = class({})
	local itemClass = item_euls

	--Passive instrinsic Bonus Modifier
	modifier_euls = class({})
	local modifierClass = modifier_euls
	local modifierName = 'modifier_euls'
	LinkLuaModifier(modifierName, "items/euls", LUA_MODIFIER_MOTION_NONE)

	--Euls MS Modifier Aura hack
	modifier_euls_ms = class({})
	local buffModifierClass = modifier_euls_ms
	local buffModifierName = 'modifier_euls_ms'
	LinkLuaModifier(buffModifierName, "items/euls", LUA_MODIFIER_MOTION_NONE)	
...
function itemClass:CastFilterResultTarget( hTarget )
	...
	local nResult = UnitFilter( hTarget, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, self:GetCaster():GetTeamNumber() )
...
function itemClass:OnSpellStart()
	...
	--[[ Add CD on Euls Lvl 2
	for i=DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_9 do
        local item = caster:GetItemInSlot(i)
 		if item then
         	if item:GetName() == "item_euls2" then
...
function buffModifierClass:GetModifierMoveSpeedBonus_Constant()
	if self:GetParent():HasModifier("modifier_euls2") then return 0 end
