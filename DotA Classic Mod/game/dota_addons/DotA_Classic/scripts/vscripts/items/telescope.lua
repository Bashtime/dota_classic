--Class Definitions
	...
	item_telescope = class({})
	local itemClass = item_telescope

	--Passive instrinsic Bonus Modifier
	modifier_telescope = class({})
	local modifierClass = modifier_telescope
	local modifierName = 'modifier_telescope'
	LinkLuaModifier(modifierName, "items/telescope", LUA_MODIFIER_MOTION_NONE)
...
function modifierClass:OnCreated()
	...
	self.bonus_str = self:GetAbility():GetSpecialValueFor( "all_stats" )
	self.bonus_agi = self:GetAbility():GetSpecialValueFor( "all_stats" )
	self.bonus_int = self:GetAbility():GetSpecialValueFor( "all_stats" )
	...
	if IsServer() then
	caster:AddNewModifier(caster, self:GetAbility(), "modifier_item_spy_gadget", { duration = -1})
	end
...
function modifierClass:OnDestroy()
	...
	if IsServer() then
	caster:RemoveModifierByName("modifier_item_spy_gadget")
	end
end
