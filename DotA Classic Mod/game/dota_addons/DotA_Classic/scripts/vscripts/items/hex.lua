--Class Definitions
	...
	item_hex = class({})
	local itemClass = item_hex

	--Passive instrinsic Bonus Modifier
	modifier_hex = class({})
	local modifierClass = modifier_hex
	local modifierName = 'modifier_hex'
	LinkLuaModifier(modifierName, "items/hex", LUA_MODIFIER_MOTION_NONE)
...
function itemClass:OnSpellStart()
	...
	target:AddNewModifier(caster, self, "modifier_sheepstick_debuff", { duration = self:GetSpecialValueFor( "sheep_duration" )})
