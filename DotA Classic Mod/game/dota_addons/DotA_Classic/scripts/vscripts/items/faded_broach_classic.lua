--Class Definitions
	...
	item_faded_broach_classic = class({})
	local itemClass = item_faded_broach_classic

	--Passive instrinsic Bonus Modifier
	modifier_broach_classic = class({})
	local modifierClass = modifier_broach_classic
	local modifierName = 'modifier_broach_classic'
	LinkLuaModifier(modifierName, "items/faded_broach_classic", LUA_MODIFIER_MOTION_NONE)
...
function modifierClass:OnCreated()
	...
	-- Extra references
	self.bonus_cr = self:GetAbility():GetSpecialValueFor( "bonus_cr" )
			...
			function modifierClass:DeclareFunctions()
				...
				--Extra: Bonus Cast Range
				function modifierClass:GetModifierCastRangeBonusStacking()
					local caster = self:GetParent()
					if caster:HasModifier("modifier_ex_machina_classic") then return 0 end
					return self.bonus_cr
				end				
