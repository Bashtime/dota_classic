--Class Definitions
	...
	item_grove_bow_classic = class({})
	local itemClass = item_grove_bow_classic

	--Passive instrinsic Bonus Modifier
	modifier_grove_bow_classic = class({})
	local modifierClass = modifier_grove_bow_classic
	local modifierName = 'modifier_grove_bow_classic'
	LinkLuaModifier(modifierName, "items/grove_bow_classic", LUA_MODIFIER_MOTION_NONE)
...
function modifierClass:OnCreated()
	...
	self.speed = self:GetAbility():GetSpecialValueFor( "projectile_speed" )
			...
			function modifierClass:DeclareFunctions()
				...
				--Extra: Bonus Attack Range and Projectile Speed
				function modifierClass:GetModifierAttackRangeBonus()
					local caster = self:GetParent()
					if caster:IsRangedAttacker() then 
						if not caster:HasModifier("modifier_ballista_classic_passive") then return self.bonus_range end
					end
				end				

				function modifierClass:GetModifierProjectileSpeedBonus()
					return self.speed
				end